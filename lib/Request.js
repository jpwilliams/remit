...
class Request extends CallableInstance {
  ...
  send (...args) {
    ...
    return ns.active ? this._send(...args) : ns.runAndReturn(this._send.bind(this, ...args))
  }

  async _send (data = null, opts = {}) {
    ...
    const ns = getNamespace('remit-breadcrumbs')
    const breadcrumbs = ns.get('crumbs') || []
    breadcrumbs.push(`${messageId} ${this._remit._options.name || 'Unnamed'} REQUEST SENT ${parsedOptions.event} :: ${this._remit._fingerprint}`)
    ns.set('crumbs', breadcrumbs)
    ...
    return new Promise((resolve, reject) => {
      ...
      types.forEach((type) => {
        this._emitter.once(`${type}-${messageId}`, (message, ...args) => {
          if (message && message.properties && message.properties.headers && message.properties.headers.crumbs) {
            message.properties.headers.crumbs.push(`${messageId} ${this._remit._options.name} ${type === 'data' ? 'RESPONSE RECV' : 'TIMED OUT'} ${this._options.event} :: ${this._remit._fingerprint}`)
            prevContext.crumbs = message.properties.headers.crumbs
          }
