...
class Request extends CallableInstance {
  ...
  async _send (data = null, opts = {}, extendedCapture = false) {
    ...
    this._remit._emitter.emit('sent', event)
  ...
  async _incoming (message) {
    ...
    this._remit._emitter.emit(`data-${message.properties.correlationId}`, message, ...content)
  }
  ...
  _waitForResult (messageId) {
    ...
    return new Promise((resolve, reject) => {
      const cleanUp = (message, err, result) => {
        ...
        if (err) {
          this._remit._emitter.emit('error', err)
          ...
          resolve(result)
          this._remit._emitter.emit('success', result, message)
        }

        this._remit._emitter.emit(
          'data',
      ...
      types.forEach((type) => {
        this._remit._emitter.once(`${type}-${messageId}`, (message, ...args) => {
          ...
          if (type !== 'data') this._remit._emitter.emit(type, ...args)
        })
      })
    })
  }
