...
class Request extends CallableInstance {
  ...
  send (...args) {
    ...
    if (!ns.active) args[2] = true

    return ns.active
      ? this._send(...args)
      : ns.runAndReturn(this._send.bind(this, ...args))
  }
  ...
  async _send (data = null, opts = {}, extendedCapture = false) {
    ...
    const ns = getNamespace('remit-breadcrumbs')
    const originId = ns.get('originId') || messageId
    const bubbleId = ns.get('bubbleId') || null
    // ns.set('originId', originId)
    // ns.set('bubbleId', bubbleId)
    ...
    }, eventData, true, true, false, 'exit')
  ...
  _waitForResult (messageId) {
    ...
    return new Promise((resolve, reject) => {
      const cleanUp = (message, err, result) => {
        ...
        this._emitter.emit('received', parseEvent(message.properties, message.fields, err || result, false, true, true))
      }
