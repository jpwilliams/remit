...
function Request (base, type, options) {
  ...
  let request = function () {
      ...
      if (expiration > 0) {
        demitQueue = new Promise((resolve, reject) => {
          ...
          remit._workChannelPool.acquire().then((channel) => {
            ...
            return remit._publishPools[options.event].acquire()
          }).catch((err) => {
        ...
        demitQueue = remit._publishPools[options.event].acquire()
      }
    } else {
      demitQueue = remit._publishPools[options.event].acquire()
    }
    ...
    demitQueue.then((publishChannel) => {
      ...
      remit._publishPools[options.event].release(publishChannel)
    ...
    return new Promise((resolve, reject) => {
      const cleanUp = (err, result) => {
        ...
        if (type.options.expectReply) {
          request._emitter.removeAllListeners(`data-${event.eventId}`)
          request._emitter.removeAllListeners(`timeout-${event.eventId}`)
        } else {
          request._emitter.removeAllListeners(`sent-${event.eventId}`)
        }

        request._emitter.removeAllListeners(`error-${event.eventId}`)
      ...
      if (type.options.expectReply) {
        request._emitter.once(`data-${event.eventId}`, cleanUp)
        type._dataCallbacks.forEach((callback) => {
          request._emitter.once(`data-${event.eventId}`, callback)
        })
        request._dataCallbacks.forEach((callback) => {
          request._emitter.once(`data-${event.eventId}`, callback)
        })

        request._emitter.once(`timeout-${event.eventId}`, cleanUp)
        type._timeoutCallbacks.forEach((callback) => {
          request._emitter.once(`timeout-${event.eventId}`, callback)
        })
        request._timeoutCallbacks.forEach((callback) => {
          request._emitter.once(`timeout-${event.eventId}`, callback)
        })
      } else {
        request._emitter.once(`sent-${event.eventId}`, a => cleanUp(null, a))
        type._sentCallbacks.forEach((callback) => {
          request._emitter.once(`sent-${event.eventId}`, callback)
        })
      ...
      request._emitter.on(`error-${event.eventId}`, cleanUp)
    })
  }
  ...
  if (!remit._publishPools[options.event]) {
    remit._publishPools[options.event] = genericPool.createPool({
      create: () => {
        return getPublishChannel.apply(remit, [type, request])
      },
