...
function __receive (skeleton, args, callback, options, masterOptions) {
  var self = this
  options = options || {}
  ...
  ret.masterOptions = masterOptions
  ...
  ret.off = function off (event, func) {
    if (event) {
      ...
      self.__assert('consume', function (consumeChannel) {
        consumeChannel.cancel(ret.consumerTag, function (err, ok) {
          if (err) {
            console.error(err)
          }
        })
  ...
  self.__assert('connection', function (connection) {
    self.__assert('exchange', function (exchange) {
      self.__assert('consume', function (consumeChannel) {
        consumeChannel.assertQueue(options.queueName || args.event, {
          exclusive: false,
          durable: true,
          autoDelete: false
        }, function (err, ok) {
          ...
          consumeChannel.bindQueue(ok.queue, self._exchange_name, args.event, {}, function (err, ok) {
            if (err) {
              throw err
            }
            ...
            consumeChannel.consume(ok.queue, function (message) {
              ...
              self.__receive_data(message, ret, masterOptions)
            }, {
              exclusive: false,
              noAck: false
            }, function (err, ok) {
