...
module.exports = function __handle_message (message, emitter) {
    ...
    const done = function done () {
        const args = ['complete'].concat(Array.from(arguments))
    ...
    emitter.once('complete', function complete () {
        ...
        // const response_data = new Buffer(JSON.stringify(Array.from(arguments)))
        const options = {correlationId: message.properties.correlationId}
        const response_data = self.__build.response(Array.from(arguments), options)
