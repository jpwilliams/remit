...
function Request (masterOptions) {
  // var self = this

  var ret = function request (args) {
    debug('Making new request', masterOptions, args)
    ...
    }, masterOptions ? masterOptions.options || {} : {})

    if (masterOptions && !masterOptions.reply) {
      delete options.replyTo
    }

    if (extraParams.length) {
      Array.from(extraParams).forEach(function (param) {
        if (callback) {
